...
void mutex_lock(pthread_mutex_t *a){ pthread_mutex_lock(a); }
void mutex_unlock(pthread_mutex_t *a){ pthread_mutex_unlock(a); }
...
#if MALLOC_DEBUG == 1 
	printf("[%s]_before calling pos_lookup_mstate(%s)\n",__func__,name) ; 	
#endif 
	ar_ptr = (struct malloc_state *)pos_lookup_mstate(name);
//#if MALLOC_DEBUG == 1 
	printf("[%s_after calling pos_lookup_mstate ar_ptr = [%p]\n",__func__,  ar_ptr) ;
//#endif 
	if (ar_ptr == NULL) {
		return NULL;
	}
...
//		printf("pos_malloc_init_state\n") ; 	
		getc(stdin) ; 	
	}
#if MALLOC_DEBUG == 1 
	printf("[%s]_before enter pos_int_malloc\n",__func__); 	
#endif 
	(void)mutex_lock(&ar_ptr->mutex);
	victim = pos_int_malloc(name, ar_ptr, bytes);
	(void)mutex_unlock(&ar_ptr->mutex);
#if MALLOC_DEBUG == 1 
	printf("[%s]_after enter pos_int_malloc\n" , __func__);
#endif
	return victim;
}
