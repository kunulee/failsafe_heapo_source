...
static void dccmvac( unsigned long mva ){ 
	/* cache flush address which is mva */
        asm volatile("mcr p15, 0, %0, c7, c10, 1" : : "r" (mva));
	
}
asmlinkage void sys_cache_flush_array(void **p,int flush_cnt){ 
	void *end ; 
//	printk("[%s][%p][%d]\n" , __func__ , p , flush_cnt) ;
	int i = 0 ; 	
	dsb() ; 	
...
asmlinkage void sys_cache_flush(void *p, unsigned long size){ 
	/* arm cache line flush */ 
	...
	for( end = p+size ; p < end ; p+=cacheline_bytes)
		dccmvac(p) ; 	
	dsb() ; /* so we know the flushes happend before continuing */ 
} 
asmlinkage int sys_pos_recovery(char __user *name){ 
       // printk("\n\n==[%s]==\n", __func__ ) ;
...
//	printk("==================\n") ; 	
	task_pid = record->task_list;
	while (task_pid) {
		if( task_pid->pid_nr > task_tgid - 10 || 
				task_pid->pid_nr < task_tgid + 10){ 
//		if( task_pid->pid_nr == task_tgid){// || 
//			task_pid->pid_nr == task_pid_nr(task)){
//		printk("tgid matching : %d  %d\n", task_pid->pid_nr , task->tgid) ; 

//		if (task_pid->pid_nr == task_pid_nr(task)) {
			break;
		} else {
//			printk("else %d %d %d\n", task_pid->pid_nr , task_pid_nr(task), task->tgid);
			task_pid = task_pid->task_next;
		}
	}
