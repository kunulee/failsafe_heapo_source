...
stm_pos_allocate_rs_entries(stm_tx_t *tx, int extend){ 
  if (extend) {
    ...
    tx->r_set.entries = (r_entry_t *)xrealloc(tx->r_set.entries, tx->r_set.size * sizeof(r_entry_t));
      ...
      tx->r_set.entries = (r_entry_t *)rs_ws_malloc_aligned( tiny_obj_store , tx->r_set.size * sizeof(r_entry_t));
  }
} 
/*
...
stm_pos_allocate_ws_entries(stm_tx_t *tx, int extend)
{
  ...
  if (extend) {
    ...
    a = tx->w_set.entries;
    /* pos_malloc */ 	
    tx->w_set.entries = (w_entry_t *)rs_ws_malloc_aligned(tiny_obj_store, tx->w_set.size*sizeof(w_entry_t)); 
...
{
//	rollback_cnt++ ; 	
//	PR_DEBUG("rollback_cnt[%d]\n", rollback_cnt++) ; 
//	PR_DEBUG("Make delayed index 0\n") ; 
#if CM == CM_BACKOFF
  unsigned long wait;
  volatile int j;
#endif /* CM == CM_BACKOFF */
...
int_stm_pos_init_thread( char *name)
{
  ...
  /* set tiny_obj_store */ 
  tiny_obj_store = (char *)malloc(sizeof(char) * 128) ; 
  memcpy( tiny_obj_store , log_name , sizeof(log_name)) ; 	

//  tiny_obj_store = log_name ; 	
  
  PR_DEBUG("[tiny_obj_store : %s]\n" , tiny_obj_store) ;
  ...
  if( tx == NULL ){ 
	PR_DEBUG("Failed to allocate tx desc in nvram\n") ; 	
	exit(-1) ; 	
  } 
  /* Set attribute */
...
//#ifdef CONFLICT_TRACKING
  ...
  /* insert list */ 
  pos_list_insert(log_name,&tx->thread_id,tx,sizeof(stm_tx_t*));
...
//  xfree(tx);
//#endif /* ! EPOCH_GC */
  pos_list_remove( log_name , &tx->thread_id ) ;
  PR_DEBUG("[%s][tx = %p]\n",log_name, tx) ; 	
  PR_DEBUG("[%p][%p]\n", tx->r_set.entries , tx->w_set.entries) ;
//  pos_free(log_name , tx->r_set.entries) ; 	
// pos_free(log_name , tx->w_set.entries) ; 
//  pos_free(log_name , tx) ;
//  tls_set_tx(NULL);
  tls_set_tx(NULL) ; 	
//  pos_list_remove_all(log_name , NULL ) ;
...
#if KEONWOO_DEBUG == 1 
  PR_DEBUG("Allocate descriptor[%p]\n" , tx) ; 	
#endif
...
void get_min_or_max(struct list_head *head , int flags){ 
//void get_min_or_max( stm_tx_t *head_tx , int flags){ 	
	...
	printf("[%s][total entries count:%d]\n", __func__, count) ; 	
	
	stm_tx_t *head_tx = (stm_tx_t *)node->value; /* head_tx is first stm_tx_t descriptor */	

	if( head_tx == NULL ){ 
		printf("[%s][head_tx:%p]\n", __func__ , head_tx ) ; 	
		return ;
	}
	w_entry_t *w = head_tx->w_set.entries; 	/* w is first stm_tx_t's write entry */ 
	if( w == NULL ){ 
		printf("[%s][w_entry_t:%p]\n", __func__ , w ) ; 	
		return ; 	
	}	
	/* according to flag value, set min value and max value */
	if( flags == 0 ){ 
		min_num = w->seq_num ; 	
		printf("[%s][tmp min_num:%d]\n",__func__,min_num); 	
	}else if( flags == 1 ){ 
		max_num = w->seq_num ; 	
		printf("[%s][tmp max_num:%d]\n",__func__,max_num) ; 	
	}
	/* during traversing stm_tx_t descriptor, get min,max value */ 	
	PR_DEBUG("[%s] traverse stm_tx_t descriptor\n", __func__ ) ; 	
	while( node != NULL ){ 
		printf("[%s] stm_tx_t[%p]\n",__func__,head_tx) ; 	
		for( i = head_tx->w_set.nb_entries ; i > 0 ; i-- , w++ ){
			if( flags == 0 ){ /* if get min number */ 
				if( min_num > w->seq_num ){ 
					min_num = w->seq_num ; 	
				}
			}else if( flags == 1 ){ /* if get max number */
		...
		head_tx = (stm_tx_t *)node->value ; /* point next stm_tx_t */ 
		w = head_tx->w_set.entries ; /* point next stm_tx_t's write entries */ 
	}

	if( flags == 0 )
		printf("[%s]final min_value[%d]\n",__func__,min_num) ; 	
	else if(flags == 1)
		printf("[%s]final max_value[%d]\n",__func__,max_num) ; 	
	/* return min_max_num */ 
} 
void look_seqnum(struct list_head *head){ 
	
	get_min_or_max(head,0) ; 	
	get_min_or_max(head,1) ; 
...
#define MAX 65535
void 
overflow_recovery(char *name, stm_tx_t *tx){ 
	/* min -> 0 , 65535 -> mx , for DS transaction consistency */ 
	PR_DEBUG("OVERFLOW RECOVERY\n") ; 	
	int i,j = 0;
...
void
int_stm_recovery(char *name, struct list_head *head){ 
...
//	PR_DEBUG("NNODE : %p\n" , node->value) ;	
	int i = 0 ; 
	stm_tx_t *tmp = NULL ;  /*tx descriptor address */ 
	while( node != NULL ){ 
		tmp = (stm_tx_t *)node->value; 
		printf("[%d]stm_tx_t[%p]\n", tmp->thread_id , tmp);	

		w_entry_t *w = tmp->w_set.entries; //get write entries set.
		for( i = tmp->w_set.nb_entries ; i > 0 ; i-- , w++){
			printf("[%d]seq_num[%d]\n", tmp->thread_id, w->seq_num);
		}

		/* next stm_tx_t descriptor */
		node = node->next ; 
		count++; 	
	}
	PR_DEBUG("[total_stm_tx_t cnt]:%d\n",count) ; 	
	/* find min,max number */
	...
	if( overflow_flags == 0 ){ /* normal case */
		normal_recovery( name, head ) ; 	
	}else if( overflow_flags == 1){ /* overflow unsigned short */ 
//		PR_DEBUG("****NOT IMPLEMENT YET*****\n") ; 
//		PR_DEBUG("    [OVERFLOW CASE]       \n") ; 	
//		sleep(10) ; 	
//		overflow_recovery(name, tx ) ; 	
	}
//	overflow_flags = 0 ; 	
	return ; 		
} 
