...
void cache_flush_write_entry( w_entry_t *w, unsigned int size){ 
	...
	for(; w < vend ; w+= 64 ){
		...
		tiny_clflush(w);
		#if KEONWOO_DEBUG == 1 	
		PR_DEBUG("[%d][%p] flush cnt : %d,seq_num[%d] \n",w->commit_mark,w,cf_cnt,w->seq_num) ; 	
		#endif 
	}	
	tiny_clflush(vend) ; 	
