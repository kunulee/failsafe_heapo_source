...
////////////////////////////////////////////////////////////////////////////////
int
tiny_pos_recovery(char *name){ 
	int a = (int)syscall(308,name) ; 	
	printf("%s\n" , __func__ ) ; 	
	return a ;
} 
int
...
pos_map(char* name)
{
	...
	while (name_entry) {
		//printf(" name_entry->name : %s\n " , name_entry->name) ; 
		if (strcmp(name, name_entry->name) == 0) {
		/*	#ifdef KEONWOO_DEBUG 1
				if( tid != pid ){
				 printf("Multithreading\n") ; 
				//syscall( __NR_pos_thread_map , name ) ; 
				printf("name : %s\n" , name) ; 
				void * ret = (void *)syscall(307 , name ) ; 	
				return 1;
				} 
				//syscall(307,name); //sys_pos_thread_map
			#endif*/
